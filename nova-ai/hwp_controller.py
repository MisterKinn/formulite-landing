...
def _is_hwp_registration_error(exc: Exception | None) -> bool:
    msg = str(exc or "")
    lower = msg.lower()
    return (
        "class not registered" in lower
        or "library not registered" in lower
        or "라이브러리가 등록되지 않았습니다" in msg
        or "클래스 팩터리" in msg
        or "0x80040154" in lower
        or "-2147221164" in lower
        or "-2147319779" in lower
    )


def _is_win32com_cache_error(exc: Exception | None) -> bool:
    text = str(exc or "")
    lower = text.lower()
    return (
        "gen_py" in lower
        or "clsidtoclassmap" in lower
        or "makepy" in lower
        or "generated files for com support" in lower
        or ("__init__.py" in lower and ("gen_py" in lower or "win32com" in lower))
    )
...
def _format_connect_error(primary_exc: Exception, secondary_exc: Exception | None) -> str:
    ...
    if _is_hwp_registration_error(primary_exc) or (
        secondary_exc is not None and _is_hwp_registration_error(secondary_exc)
    ):
        return (
            "HWP 연결 실패: 한글 오토메이션(COM) 라이브러리를 찾지 못했습니다. "
            "한글 2022 설치/복구 여부를 확인하고, LitePro와 HWP를 같은 권한으로 실행해 주세요."
        )
...
class HwpController:
    ...
    def _get_com_page_hint() -> tuple[int, int]:
        ...
        try:
            ...
            hwp, _ = HwpController._attach_running_hwp_object()
            if hwp is None:
                return (0, 0)
    ...
    @staticmethod
    def _ensure_win32com_cache_package() -> None:
        """Create missing win32com cache package files without deleting the cache."""
        try:
            import win32com  # type: ignore
    ...
    @staticmethod
    def _get_active_hwp_via_rot() -> tuple[Any | None, Exception | None]:
        """Attach to a running HWP object through the Running Object Table."""
        try:
            import pythoncom  # type: ignore
            import win32com.client  # type: ignore
            ...
            while True:
                ...
                if "!HwpObject." not in display_name:
                    continue
                try:
                    return win32com.client.Dispatch(rot.GetObject(moniker)), None
    ...
    @staticmethod
    def _get_active_hwp_via_progid() -> tuple[Any | None, Exception | None]:
        """Attach to a running HWP object through known ProgIDs."""
        try:
            import win32com.client  # type: ignore
    ...
    @staticmethod
    def _attach_running_hwp_object() -> tuple[Any | None, Exception | None]:
        """Best-effort attach to an already-running HWP instance."""
        hwp_obj, exc = HwpController._get_active_hwp_via_rot()
        if hwp_obj is not None:
            return hwp_obj, None

        hwp_obj, progid_exc = HwpController._get_active_hwp_via_progid()
        if hwp_obj is not None:
            return hwp_obj, None
    ...
    def _register_security_module_best_effort(self) -> None:
        ...
        if not self._register_module or self._hwp is None:
            return
        register_module = getattr(self._hwp, "RegisterModule", None)
        ...
        for module_name in (
            ...
            try:
                if register_module("FilePathCheckDLL", module_name):
                    return
            except Exception:
                continue
    ...
    def connect(self) -> None:
        ...
        with _preserve_foreground():
            hwp_obj, attach_exc = self._attach_running_hwp_object()
                ...
                try:
                    ...
                    if _is_win32com_cache_error(exc):
                        ...
                        if not self.find_hwp_windows():
                            raise HwpControllerError(
                                "한글(HWP) 창을 찾지 못했습니다. 먼저 HWP를 실행해 주세요."
                            ) from exc
                        raise HwpControllerError(
                            _format_connect_error(exc, attach_exc)
                        ) from exc

            self._hwp = hwp_obj
            self._register_security_module_best_effort()
    ...
    def get_active_document_name(self) -> str:
        """Best-effort: read the active HWP document title from COM."""
        try:
            windows = self._ensure_connected().XHwpWindows
        except Exception:
            return ""
        ...
        win = None
        for attr in ("Active_XHwpWindow", "ActiveXHwpWindow", "ActiveWindow"):
            try:
                candidate = getattr(windows, attr)
            except Exception:
                candidate = None
        ...
        if win is None:
            for method_name in ("get_Active_XHwpWindow", "GetActive_XHwpWindow"):
                method = getattr(windows, method_name, None)
        ...
        if win is None:
            try:
                if int(getattr(windows, "Count", 0) or 0) == 1:
                    win = windows.Item(0)
            except Exception:
                win = None

        title = self._get_hwp_win_title(win) if win is not None else ""
        return self._extract_doc_name_from_title(title) or title.strip()
